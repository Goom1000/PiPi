...
must_haves:
  truths:
    - "User can see matched slides after enhancement completes"
...
3. Configuration Section (shown when status === 'idle'):
   - Header: "Enhance this resource"
   - Brief description: "{resource.name} will be enhanced with three differentiation levels"
   - "Enhance" button (primary, full-width)
   - Note: Slide matching happens during enhancement (single API call for efficiency)
